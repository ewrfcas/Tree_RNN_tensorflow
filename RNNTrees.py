...
class RecursiveNetStaticGraph():
    def __init__(self, config):
        ...
        with tf.variable_scope('Composition'):
            W1 = tf.get_variable('W1',[2 * (self.config.embed_size+2) , self.config.embed_size+2])
        ...
        def combine_children(left_tensor, right_tensor):
            return tf.nn.relu(tf.matmul(tf.concat([left_tensor, right_tensor], axis=1), W1) + b1)
        ...
        self.lr = tf.minimum(self.config.lr,
                             self.config.lr / tf.log(9999.) * tf.log(tf.cast(self.global_step, tf.float32) + 1))
